...
- 26 local skills exist under `.agents/skills`.
...
- Advisory role playbooks exist under `.agents/subagents` for BTP, Integration Suite, CAP, Fiori, UI5 quality, HANA, SQLScript, API style, and identity security.
...
|---|---|---|
| BTP Platform Advisor | `sap-btp-cloud-platform`, `.agents/subagents/sap-btp-platform-advisor.md` | Account model, subaccounts, entitlements, CF/Kyma/ABAP environment, roles, operations |
...
| Integration Suite Advisor | `sap-btp-integration-suite`, `.agents/subagents/sap-integration-flow-advisor.md` | iFlows, API Management, Event Mesh, B2B/EDI, transport readiness, monitoring |
| Identity Security Advisor | `.agents/subagents/sap-identity-security-advisor.md`, `sap-btp-cloud-platform`, `sap-btp-connectivity` | IAS/IPS trust, role collections, authorization mapping, auditability |
| HANA Database Advisor | `sap-hana-cli`, `.agents/subagents/sap-hana-database-advisor.md` | HANA inspection, HDI, service bindings, database readiness |
...
| CAP Project Architect | `sap-cap-capire`, `.agents/subagents/sap-cap-project-architect.md` | CAP services, CDS, handlers, deployment, multitenancy, Fiori integration |
| Fiori App Advisor | `sap-fiori-tools`, `.agents/subagents/sap-fiori-app-advisor.md` | Fiori Elements, app generation, annotations, preview, deployment |
| UI5 Quality Advisor | `sapui5`, `.agents/subagents/sap-ui5-code-quality-advisor.md` | SAPUI5/OpenUI5 architecture, routing, binding, accessibility, tests, performance |
| SQLScript Analyzer | `sap-sqlscript`, `.agents/subagents/sap-sqlscript-analyzer.md` | Procedures, table functions, AMDP, exception handling, HANA performance |
| API Style Reviewer | `.agents/subagents/sap-api-style-reviewer.md` | API consistency and consumer-facing contract review when APIs are in scope |
...
Create `.agents/AGENT_REGISTRY.md` from this plan, then add `.agents/playbooks/` for the seven high-value routing paths. That will turn the current skill inventory into a practical operating layer without enabling risky automation.
